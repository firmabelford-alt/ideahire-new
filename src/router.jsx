...
const PAYMENT_STATUS_DETAILS = Object.freeze({
  awaiting_payment: {
  ...
  processing: {
    label: "Płatność jest przetwarzana",
    tone: "waiting",
  },
  funds_secured: {
    label: "Środki są zabezpieczone",
    tone: "success",
  },
  work_submitted: {
    label: "Praca została przekazana",
    tone: "success",
  },
  release_pending: {
    label: "Wypłata jest przygotowywana",
    tone: "waiting",
  },
  released: {
    label: "Środki zostały przekazane",
    tone: "success",
  },
  refund_pending: {
    label: "Zwrot jest przygotowywany",
    tone: "waiting",
  },
  partially_refunded: {
    label: "Wykonano częściowy zwrot",
    tone: "neutral",
  },
  refunded: {
    label: "Zwrot został wykonany",
    tone: "neutral",
  },
  disputed: {
    label: "Płatność objęta sporem",
    tone: "danger",
  },
  failed: {
    label: "Płatność nie powiodła się",
    tone: "danger",
  },
  cancelled: {
    label: "Płatność została anulowana",
    tone: "neutral",
  },
});

function formatPaymentAmount(value, currency = "PLN") {
...
function PaymentPanel({
  ...
  return (
    <section className="payment-panel">
      <div className="payment-panel-heading">
        <div className="payment-panel-title">
          ...
          <div>
            ...
            <h2>Bezpieczne rozliczenie</h2>
            <p>
              Kwoty zostały obliczone i zapisane po zaakceptowaniu warunków przez obie strony.
            </p>
          </div>
        </div>
        ...
        {payment && (
          <span
            className={`payment-status is-${status.tone}`}
          >
            <i aria-hidden="true" />
            {status.label}
          </span>
      ...
      {loading && !payment ? (
        <div className="payment-panel-feedback">
          <span
            className="payment-feedback-spinner"
            aria-hidden="true"
          />
          Ładowanie podsumowania płatności...
        </div>
      ) : error ? (
        <div
          className="payment-panel-feedback is-error"
          role="alert"
        >
          <span>{error}</span>
          <button
        ...
        <>
          <div className="payment-breakdown">
            <div className="payment-amount-card">
              ...
              <small>
                Pełna kwota wynagrodzenia wykonawcy
              </small>
            </div>
            ...
            <div className="payment-amount-card">
              ...
              <small>
                7,5% · minimum 20 zł · maksimum 300 zł
              </small>
            </div>

            <div className="payment-amount-card is-contractor">
              <span>Wykonawca otrzyma</span>
              <strong>
                {formatPaymentAmount(
                  payment.contractor_amount,
                  currency
                )}
              </strong>
              <small>
                Bez potrącania opłaty IdeaHire
              </small>
            </div>
          </div>
          ...
          <div className="payment-total-row">
            <div>
              ...
              <small>
                Cena zlecenia wraz z opłatą IdeaHire
              </small>
            </div>
          ...
          <div className="payment-next-step">
            <div>
              <strong>
                {isClient
                  ? "Płatność nie została jeszcze pobrana"
                  : "Czekamy na płatność zleceniodawcy"}
              </strong>
              <p>
                {isClient
                  ? "Bezpieczna płatność online zostanie uruchomiona w następnym etapie wdrożenia."
                  : "O rozpoczęciu i zabezpieczeniu płatności poinformujemy Cię w tym miejscu."}
              </p>
            </div>
        ...
        <div className="payment-panel-feedback">
          <span>
            Przygotowujemy podsumowanie płatności dla zaakceptowanych warunków.
          </span>
          <button
            type="button"
            onClick={onRefresh}
...
function Chat() {
  ...
  async function loadPaymentSummary(
    ...
    try {
      ...
      setPaymentError(
        "Nie udało się pobrać podsumowania płatności. Odśwież stronę i spróbuj ponownie."
      );
      return null;
    } finally {
      setPaymentLoading(false);
      ...
      <main className="chat-page">
        <style>{`
          ...
          .payment-panel {
            ...
            border-radius: 20px;
            background: linear-gradient(145deg, #fff 0%, #fafaf7 100%);
          ...
          .payment-amount-card.is-contractor {
            border-color: #d7e7da;
            background: #f4faf5;
          }
          ...
          .payment-refresh-button:hover,
          .payment-panel-feedback button:hover {
            border-color: #aaa69d;
            background: #fdfdfb;
          }

          .payment-refresh-button:disabled,
          .payment-panel-feedback button:disabled {
            cursor: wait;
            opacity: .58;
          }
          ...
          .payment-panel-feedback.is-error {
            border-color: #efd4cf;
            background: #fff5f3;
            color: #913d34;
          }

          .payment-feedback-spinner {
            width: 15px;
            height: 15px;
            flex: 0 0 15px;
            border: 2px solid rgba(70,70,65,.22);
            border-top-color: #4b4a45;
            border-radius: 50%;
          ...
          html[data-theme="dark"] .payment-panel {
            border-color: #3a3a36;
            background: linear-gradient(145deg, #1d1d1b 0%, #171716 100%);
            box-shadow: 0 14px 36px rgba(0,0,0,.2);
          }

          html[data-theme="dark"] .payment-shield {
            border-color: #34543c;
            background: #203427;
            color: #a8d8b2;
          }
          ...
          html[data-theme="dark"] .payment-status.is-success {
            border-color: #36543d;
            background: #203427;
            color: #a8d8b2;
          }

          html[data-theme="dark"] .payment-status.is-danger {
            border-color: #603a35;
            background: #392421;
            color: #efaaa1;
          }

          html[data-theme="dark"] .payment-amount-card {
            border-color: #353532;
            background: rgba(39,39,36,.78);
          }

          html[data-theme="dark"] .payment-amount-card.is-contractor {
            border-color: #34513a;
            background: #203126;
          }

          html[data-theme="dark"] .payment-total-row {
            border: 1px solid #4b4b46;
            background: #f0f0eb;
            color: #171717;
          }

          html[data-theme="dark"] .payment-total-row span {
            color: #171717;
          }

          html[data-theme="dark"] .payment-total-row small {
            color: #686862;
          }

          html[data-theme="dark"] .payment-next-step,
          html[data-theme="dark"] .payment-panel-feedback {
            border-color: #3e3d38;
            background: #25241f;
          }

          html[data-theme="dark"] .payment-next-step strong {
            color: #efefe9;
          }

          html[data-theme="dark"] .payment-next-step p,
          html[data-theme="dark"] .payment-panel-feedback {
            color: #b2b0a8;
          }

          html[data-theme="dark"] .payment-refresh-button,
          html[data-theme="dark"] .payment-panel-feedback button {
            border-color: #4a4944;
            background: #30302c;
            color: #efefe9;
          }

          html[data-theme="dark"] .payment-panel-feedback.is-error {
            border-color: #603a35;
            background: #392421;
            color: #efaaa1;
          }
